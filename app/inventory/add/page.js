...
export default function AddProductPage() {
  ...
  return (
    <div className="add-page">
            ...
            {Number(form.buyPrice) > 0 && Number(form.sellPrice) > 0 && (
              <div className="profit-preview">
                <div className="profit-item">
                  ...
                  <span className={`profit-value ${profit >= 0 ? 'green' : 'red'}`}>{profit >= 0 ? '+' : ''}₹{profit}</span>
                </div>
                <div className="profit-item">
                ...
                <div className="profit-item">
                  ...
                  <span className={`profit-value ${profit >= 0 ? 'green' : 'red'}`}>₹{(profit * (Number(form.quantity) || 0)).toLocaleString('en-IN')}</span>
                </div>
              </div>
            )}
      ...
      <style jsx>{`
        .error-alert { padding: 12px; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2); border-radius: 12px; color: #EF4444; font-size: 14px; text-align: center; margin-bottom: 8px; }
        ...
        .mode-btn { flex: 1; padding: 10px; border: none; background: transparent; color: #A0A0B8; font-size: 13px; font-weight: 600; border-radius: 10px; cursor: pointer; transition: all 0.2s; }
        .mode-btn.active { background: #7B42C4; color: white; }
        ...
        .select-field {
          height: 52px; padding: 0 14px; font-size: 15px; background: #1A1A30; color: white;
        ...
        .bulk-row {
          ...
          padding: 12px 14px; background: #252540; border-radius: 12px;
        ...
        .bulk-name { font-size: 14px; font-weight: 600; color: white; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .bulk-stock { font-size: 12px; color: #6B6B80; margin-top: 2px; }
        .bulk-controls { display: flex; align-items: center; gap: 6px; flex-shrink: 0; }
        .bulk-plus { color: #22C55E; font-size: 16px; font-weight: 700; }
        .bulk-input {
        ...
        .bulk-unit { font-size: 12px; color: #6B6B80; min-width: 24px; }
        .empty-bulk { text-align: center; padding: 40px; color: #A0A0B8; }
        .empty-bulk p:first-child { font-size: 40px; margin-bottom: 12px; }
        .loading { color: #A0A0B8; text-align: center; padding: 40px; }
      `}</style>
    </div>
  );
}
